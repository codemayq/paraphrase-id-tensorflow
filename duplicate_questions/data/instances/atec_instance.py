...
class AtecInstance(STSInstance):
    def __init__(self, first_sentence, second_sentence, label, line_id, tokenizer=ChineseWordTokenizer):
        ...
        super(AtecInstance, self).__init__(first_sentence, second_sentence, label, tokenizer)
    ...
    def read_from_line(cls, line):
        ...
        if len(fields) == 4:
            ...
            line_id, first_sentence, second_sentence, label = fields
            label = int(label)
            line_id = int(line_id)
        elif len(fields) == 3:
            # test set instance
    ...
    def to_indexed_instance(self, data_indexer):
        ...
        return AtecIndexInstance(indexed_first_sentence,
                                  indexed_second_sentence,
                                  self.label_mapping[self.label],self.line_id )


class AtecIndexInstance(IndexedSTSInstance):
    def __init__(self, first_sentence_indices, second_sentence_indices, label, line_id):
        super(AtecIndexInstance, self).__init__(first_sentence_indices, second_sentence_indices, label)
        self.line_id = line_id
